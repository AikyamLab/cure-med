# -------------------------------
# Qwen2.5-7B-Instruct Fine-Tuned Batch Inference Script for Test Datasets
# -------------------------------
# This script performs batch inference on a fine-tuned Qwen2.5-7B-Instruct model using test datasets
# from multiple languages. It loads the model in full precision, generates responses
# for medical queries, and saves results progressively to CSV files. Supports resuming partial runs
# and extending to more samples.
import os
import pandas as pd
...
# -------------------------------
# List of supported languages for processing (based on JSONL filenames without '_sft.jsonl').
languages = [
...
def generate_response(question, language, max_new_tokens=1024):
    ...
    messages = [
        ...
        {"role": "user", "content": f"TThe question is in {language}. {question} Please think carefully with English-guided reasoning and code-switching, return your reasoning inside <thinking> </thinking> tags, and the final direct answer inside <answer> </answer> tags. Final answer ONLY in the language of the question."},
    ]
