#!/bin/bash -l
#SBATCH --job-name=medical_grpo_stage1_low
#SBATCH --output=logs/medical_grpo_stage1_low_%j.out
#SBATCH --error=logs/medical_grpo_stage1_low_%j.err
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
output_dir="stage_one_low_output_${uid}"
...
python -m accelerate.commands.launch \
  ...
  --rdzv_backend c10d \
  training_stage_one_low.py \
  --dataset_path "/standard/AikyamLab/eric/Multilingual medical reasoning/conversational finetuning/NEW_ALGORITHM/RFT_WITH_GRPO/Dataset_stages/stage_one/low" \
  --block_size 2048 \
