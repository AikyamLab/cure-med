#!/bin/bash -l
#SBATCH --job-name=medical_grpo_stage1_medium
#SBATCH --output=logs/medical_grpo_stage1_medium_%j.out
#SBATCH --error=logs/medical_grpo_stage1_medium_%j.err
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
#SBATCH --account=aikyamlab
#SBATCH --mail-type=END,FAIL
#SBATCH --mail-user=reh6ed@virginia.edu
...
output_dir="stage_one_medium_output_${uid}"

# Navigate to working directory (adjust if needed)
cd "/standard/AikyamLab/eric/Multilingual medical reasoning/conversational finetuning/NEW_ALGORITHM/RFT_WITH_GRPO/Training_Stages/Stage_one_training"
...
python -m accelerate.commands.launch \
  ...
  --rdzv_backend c10d \
  training_stage_one_medium.py \
  --dataset_path "/standard/AikyamLab/eric/Multilingual medical reasoning/conversational finetuning/NEW_ALGORITHM/RFT_WITH_GRPO/Dataset_stages/stage_one/medium" \
  --block_size 2048 \
