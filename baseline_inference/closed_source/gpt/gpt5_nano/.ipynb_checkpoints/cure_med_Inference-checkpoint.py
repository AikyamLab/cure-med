...
# Model name
model_name = "gpt-5-nano"  # snapshot is valid for GPT-5 mini :contentReference[oaicite:1]{index=1}
...
def generate_response(question, max_new_tokens=1024, max_retries=4):
    """
    ...
    for attempt in range(1, max_retries + 1):
        try:
            resp = client.responses.create(
                model=model_name,
                input=question,                 # simplest valid input for Responses :contentReference[oaicite:2]{index=2}
                instructions=system_prompt,     # correct place for system/developer prompt :contentReference[oaicite:3]{index=3}
                reasoning={"effort": "minimal"},# gpt-5-mini supports minimal/low/medium/high; not "none" :contentReference[oaicite:4]{index=4}
