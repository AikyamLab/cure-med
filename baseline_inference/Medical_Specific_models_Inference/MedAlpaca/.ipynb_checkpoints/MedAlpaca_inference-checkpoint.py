# -------------------------------
# Medalpaca-7B Batch Inference Script for Test Datasets
# -------------------------------
...
# Set pad token to eos token (common fix for Llama-based models without a dedicated pad token)
tokenizer.pad_token = tokenizer.eos_token
model.config.pad_token_id = tokenizer.pad_token_id

# Base system prompt template (dynamic per language, adapted to Alpaca-style format)
system_prompt_template = "### Instruction:\nYou are an expert multilingual medical doctor. The following query is in {lang}. Please reason step by step in {lang} only and provide the correct final answer to this medical query in {lang} only.\n\n### Input:\n{question}\n\n### Response:\n"

# Function to generate response
def generate_response(question, lang, max_new_tokens=512):
    prompt = system_prompt_template.format(lang=lang, question=question)
...
if __name__ == "__main__":
    ...
    # Output directory
    output_dir = "medalpaca_13b_inference_testdataset"
    ...
    for lang in process_languages:
        ...
        new_filename = f"{lang}_medalpaca_7b_inference_data.csv"
