...
# ----------------------------- Argument Parsing -----------------------------
parser = argparse.ArgumentParser(description="Generate test responses for multilingual medical QA datasets using MMed-Llama-3-8B-EnIns (vLLM + 2 GPUs).")
parser.add_argument(
    "--language",
    type=str,
    default="all",
...
parser.add_argument(
    "--model_name",
    type=str,
    default="Henrychur/MMed-Llama3.1-70B", 
    help="Hugging Face model name (default: MMed-Llama-3-8B-EnIns)."
)
parser.add_argument(
...
model_folder_name = args.model_name.split("/")[-1] + "_results"  # e.g., MMed-Llama-3-8B-EnIns_results
...
sampling_params = SamplingParams(
    ...
    stop=["<|eot_id|>"]  # Clean Llama-3 stop
)

# ----------------------------- Response Generation Function -----------------------------
def get_model_response(query: str, lang: str) -> str:
    """
    Uses official Llama-3 chat template + direct-response prompt.
    Generates clean, deterministic response with vLLM.
