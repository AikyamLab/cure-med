...
# ----------------------------- Argument Parsing -----------------------------
parser = argparse.ArgumentParser(description="Generate test responses for multilingual medical QA datasets using HuatuoGPT-o1-70B (vLLM + 2 GPUs).")
parser.add_argument(
    "--language",
    type=str,
    default="all",
...
llm = LLM(
    ...
    dtype="float16",  # Explicit FP16 for memory efficiency
    trust_remote_code=True,
    max_model_len=8192,  # Safe for long reasoning chains
    gpu_memory_utilization=0.95,  # Leave 5% VRAM free for stability
    enforce_eager=True,  # Disable graphs to reduce peak memory
    swap_space=32  # Use CPU RAM for temp during loading
)
...
[your final answer only, no additional text]"""
    ...
    for attempt in range(3):
        try:
            ...
            torch.cuda.empty_cache()  # Clear any temp allocations
            time.sleep(5)
