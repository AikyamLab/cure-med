...
def get_language_evaluation(question, response, test_response, language):
    """
...
You are provided with a question in {language}, a model-generated response (<Model Response>), and a test response (<Test Response>).
Determine if the model response and test response are entirely in {language} (the language of the question)? Check for no mixing with other languages, no unnecessary repetitions, no drifts, and a proper conclusion to the question. Your task is to simply output "True" if the criteria is met, and "False" otherwise.
"""
...
for lang in selected_languages:
    ...
    output_csv_path = os.path.join(output_dir, f"{lang}_minstral-8b_language_evaluation.csv")
    ...
    for idx in tqdm(unevaluated_indices):
        ...
        language_evaluation = get_language_evaluation(question, response, test_response, lang)
