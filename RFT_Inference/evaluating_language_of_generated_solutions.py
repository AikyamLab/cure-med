...
parser.add_argument("--language", type=str, default=None, help="Language to evaluate (e.g., French, Spanish, etc.). Required if --all is not used.")
...
dataset_dir = "/standard/AikyamLab/eric/Multilingual medical reasoning/conversational finetuning/NEW_ALGORITHM_3RD/QWEN2.5_1.5B-INSTRUCT/RFT_INFERENCE/third_evals/finetuned_qwen2.5_7B_Instruct_inference_testdataset"
...
Determine if the model response (final answer) is entirely in {language} (the language of the question)? Check for no mixing with other languages in the final answer, no unnecessary repetitions. Your task is to simply output "True" if the criteria is met, and "False" otherwise.
"""
...
for lang in selected_languages:
    # Lowercase the language for file naming consistency
    lang_lower = lang.lower()
    input_csv_path = os.path.join(dataset_dir, f"{lang_lower}_finetuned_qwen2.5_7B_instruct_inference_data.csv")
    output_csv_path = os.path.join(output_dir, f"{lang_lower}_qwen2.5_7B_language_evaluation.csv")
