...
# List of languages to process (capitalized for consistency with user input)
languages = [
...
def get_logic_evaluation(answer, test_response):
    """
...
for lang in selected_languages:
    ...
    output_csv_path = os.path.join(output_dir, f"{lang_lower}_qwen2.5_7B_logic_evaluation.csv")
    ...
    for idx in tqdm(unevaluated_indices):
        ...
        if not all(isinstance(x, str) for x in [answer, test_response]):
            print(f"Skipping invalid row at index {idx} for {lang}")
            continue
        
        logic_evaluation = get_logic_evaluation(answer, test_response)
